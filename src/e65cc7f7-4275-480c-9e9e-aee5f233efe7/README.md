...
class Item << (F,LightGreen) >> #RoyalBlue {
  ...
  # getTargetLabel(int $value, string $fallback = 'error: empty target') : string
  # getAccessLabel(int $value, string $fallback = 'error: empty access repo') : string
  # getUrl(object $item) : string
  # getDetailsTable(object $item) : string
  - getValue(object $item, string $prop, ...) : string
}
...
note right of Item::getTargetLabel
...
note right of Item::getAccessLabel
...
note right of Item::getDetailsTable
...
> Simply use this SPK:
```
Super---e65cc7f7_4275_480c_9e9e_aee5f233efe7---Power
```
